/* ============================================================
   ...
   ------------------------------------------------------------
   - Separate from the normal /api/buy-data route
   - ONLY for company = mayconnect
   - Pending transactions DO NOT debit wallet
   - Failed transactions DO NOT debit wallet
   - Successful transactions debit wallet
   - Cashback ONLY for Mayconnect
   - Maximum 5 cashback credits per Lagos day
   - ₦10 cashback per eligible successful purchase
   ============================================================ */
...
// IMPORTANT:
// These must already exist in your main server.js.
// We will inject them when registering this router.
let pool;
let auth;
let buyDataLimiter;
let callMaitamaData;
let callCheapDataHubData;
let callSubPadiData;
let callArrahuzData;
let callJJDataSubData;
let callAlihsanData;
let getCompanyAdmin;
let sendWalletUpdate;
let sendPushNotification;

function configureMayconnectBuyData(config) {
  pool = config.pool;
  auth = config.auth;
  buyDataLimiter = config.buyDataLimiter;

  callMaitamaData = config.callMaitamaData;
  callCheapDataHubData = config.callCheapDataHubData;
  callSubPadiData = config.callSubPadiData;
  callArrahuzData = config.callArrahuzData;
  callJJDataSubData = config.callJJDataSubData;
  callAlihsanData = config.callAlihsanData;

  getCompanyAdmin = config.getCompanyAdmin;
  sendWalletUpdate = config.sendWalletUpdate;
  ...
  async (req, res) => {
    ...
    try {
      ...
      /* ========================================================
         BALANCE CHECK AGAIN
         --------------------------------------------------------
         Because pending transactions did not debit,
         the user's wallet may have changed while provider
      ...
      if (currentBalance < priceNum) {
        ...
        console.error(
          "MAYCONNECT SUCCESS BUT INSUFFICIENT WALLET:",
          {
            user_id:
              user.id,
            transaction_id:
        ...
        /*
         * DO NOT mark this as FAILED because provider
         * already reported SUCCESS.
         *
         * Keep it PENDING so it can be handled/reconciled
         * by an admin/status process.
         */
        ...
        return res.status(409).json({
          ...
          message:
            "Transaction was successful at the provider, but your wallet balance is currently insufficient. Please contact support.",
      ...
      /*
       * This route is already Mayconnect-only,
       * but we keep the check explicit.
       */
