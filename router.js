...
// DATABASE POOL
// -------------------------
// We'll get the pool from server.js via export
const { pool } = require("./server"); // make sure server.js exports { pool }
...
// -------------------------
// DATA PLANS (FULL, COMPLETE)
// -------------------------
const DATA_PLANS = [
  // Mai Tama / MTN
  { plan_id: 153, network: "MTN", type: "SME", price_range: "₦1400-1500", size: "5 GB", duration: "1 month" },
  { plan_id: 13, network: "MTN", type: "GIFTING", price_range: "₦230-300", size: "1 GB", duration: "1 month" },
  { plan_id: 14, network: "MTN", type: "GIFTING", price_range: "₦530-600", size: "2 GB", duration: "1 month" },
  { plan_id: 15, network: "MTN", type: "GIFTING", price_range: "₦530-600", size: "3 GB", duration: "1 month" },
...
router.post("/login", async (req, res) => {
  ...
  if (!(await bcrypt.compare(password, r.rows[0].password))) return res.status(401).json({ error: "Invalid credentials" });
