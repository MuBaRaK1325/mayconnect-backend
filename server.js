...
/* =========================
GET PLANS (FILTERED + ADMIN PRICE)
========================= */
...
app.get("/api/plans",auth,async(req,res)=>{
  ...
  // Admin sees original price
  if(req.user.is_admin){
    return res.json(plans.rows)
  }

  // Users see price + profit margin
  const modified = plans.rows.map(p=>({
    ...p,
    price: Number(p.price) // keep selling price
  }))
...
app.post("/api/buy-airtime",auth,async(req,res)=>{
  try{
    ...
    if(Number(user.rows[0].wallet_balance) < amount){
      return res.status(400).json({message:"Insufficient balance"})
    }
