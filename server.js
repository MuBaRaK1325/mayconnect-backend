...
app.get("/api/transactions", auth, async (req, res) => {
  const tx = await pool.query("SELECT * FROM transactions WHERE user_id=$1 ORDER BY id DESC", [req.user.id]);
  res.json(tx.rows);
});
...
app.post("/api/buy-data", auth, async (req, res) => {
  ...
  if (user.rows[0].wallet_balance < price) return res.status(400).json({ message: "Insufficient balance" });
  ...
  await pool.query("INSERT INTO transactions(user_id,type,amount,phone,reference,status) VALUES($1,'DATA',$2,$3,$4,'SUCCESS')",
    [user.rows[0].id, price, phone, ref]);
...
app.post("/api/buy-airtime", auth, async (req, res) => {
  ...
  await pool.query("INSERT INTO transactions(user_id,type,amount,phone,reference,status) VALUES($1,'AIRTIME',$2,$3,$4,'SUCCESS')",
    [user.rows[0].id, amount, phone, ref]);
...
app.post("/api/admin/withdraw", auth, async (req, res) => {
  ...
  const user = await pool.query("SELECT * FROM users WHERE username=$1 AND company=$2", [username, req.user.company]);
  if (!user.rows.length) return res.status(400).json({ message: "User not found" });
  if (user.rows[0].wallet_balance < amount) return res.status(400).json({ message: "Insufficient balance" });
...
app.post("/api/admin/add-top-user", auth, async (req, res) => {
  ...
  await pool.query("UPDATE users SET is_top_user=true WHERE email=$1 AND company=$2", [email, req.user.company]);
  res.json({ message: "Top user added" });
});
app.post("/api/admin/remove-top-user", auth, async (req, res) => {
  if (!req.user.is_admin) return res.status(403).json({ message: "Forbidden" });
  const { email } = req.body;
  await pool.query("UPDATE users SET is_top_user=false WHERE email=$1 AND company=$2", [email, req.user.company]);
...
app.post("/api/admin/reverse", auth, async (req, res) => {
  ...
  if (tx.rows[0].status === "REVERSED") return res.status(400).json({ message: "Already reversed" });
